...
while jogando:
    ...
    # Jogada do oponente
        ...
        pergunta_ataque_oponente = True
        while pergunta_ataque_oponente:
            linha_atacar = random.randint(0,9)
            coluna_atacar = random.randint(0,9)
            coordenada = [linha_atacar,coluna_atacar]
            if coordenada not in ja_perguntou_oponente:
                ja_perguntou_oponente.append(coordenada)
                # Atualiza tabuleiro do oponente
                print('Seu oponente está atacando na linha {0} e coluna {1}'.format(linha_atacar,coluna_atacar))
                tabuleiro_jogador = faz_jogada(tabuleiro_jogador,linha_atacar,coluna_atacar)
                pergunta_ataque_oponente = False   

        ganhou_oponente = afundados(frota,tabuleiro_jogador) 
        if ganhou_oponente== 10:
